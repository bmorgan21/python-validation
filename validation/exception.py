...
class ValidationException(ValueError):
    ...
    def __str__(self):
        if self.error_dict is not None:
            return ', '.join(['%s.%s: %s' % (y.table, x, y) for x,y in self.error_dict.items()])
    ...
    def create_warning(cls, warning_level, name, value, *args, **kwargs):
        ...
        kwargs['warning_key'] = sha1(('%s-%s-%s-%s' % (warning_level, name, value, args[0])).encode('utf-8')).hexdigest()
        return ValidationException(*args, **kwargs)

    @staticmethod
    def merge_errors(errors, e, prefix=None):
        if not e: return errors
